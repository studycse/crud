...
$id=$_GET['id'];
	...
	$detail="SELECT * FROM registertable2 where id=$id";
	$train="SELECT * FROM training where ref_master_id=$id ";
	...
	if($res = mysqli_num_rows($result) > 0){
		...
		echo '<br> Division					: '.$res['division'];
		echo '<br> district					: '.$res['district'];
		echo '<br> upozila					: '.$res['upozila'];
